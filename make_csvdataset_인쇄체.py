...
def write_csv_datafile(imgfname, jsonfname, file):
    ...
    annot_list = []
    if is_worddata(jsonfname) == False:
        '''
        def is_worddata(jsonfname):
            with open(jsonfname, encoding='utf-8') as json_file:
                json_data = json.load(json_file)
        ...
        with open(jsonfname, encoding='utf-8') as json_file:
            ...
            for i in range(c_cnt):
                box = word[i]['letter'][0]["charbox"]
                val = word[i]['letter'][0]["value"]
                x1 = box[0]
                y1 = box[1]
                x2 = box[2]
                y2 = box[3]
...
def main():
    ...
    parser.add_argument("--data_dir", type=str, default="./_data/train_data2",
                        help="File path to data file")
    parser.add_argument("--csv_dir", type=str, default="./_data/train_data2",
                        help="File path to save CSV file")
    parser.add_argument("--val", type=float, default=0.2,
                        help="validation list ratio. default = 0.2")
    parser.add_argument("--kfold", type=int, default=5,
                        help="make validation list kfold. default = 1")

    args = parser.parse_args()

    imglist, jsonlist = get_filelist(args.data_dir)  # 함수
    ...
    if args.kfold > 1:
        ...
        for k in range(args.kfold):
            ...
            st = valcnt*k  # valcnt = int(len(imglist)/args.kfold)
            en = valcnt*(k+1)
            for i in range(len(indics)):
                if i >= st and i < en:
                    write_csv_datafile(
                        imglist[indics[i]], jsonlist[indics[i]], fval)  # indics = np.arange(len(imglist))
                    '''
                        write_csv_datafile
                        #annot_list = []
                        if is_worddata(jsonfname) == True:
                            with open(jsonfname, encoding='utf-8') as json_file:
                                ...
                                for i in range(c_cnt):
                                    ...
                                    data = imgfname + ',' + \
                                        str(x1) + ',' + str(y1) + ',' + str(x2) + \
                                        ',' + str(y2) + ",HandWrited_KR" + "\n"
                                    # annot_list.append(data)
                                    file.write(data)
                        # return annot_list
                    '''
                    write_listfile(imglist[indics[i]],   # indics = np.arange(len(imglist))
                                   jsonlist[indics[i]], feval)  # fval = open(args.csv_dir + '/validdata_'+str(k)+'.csv', "w")
                    '''
                    def write_listfile(imgfname, jsonfname, file):
                        data = imgfname + ',' + jsonfname + "\n"
                        file.write(data)
                    ...
                    write_csv_datafile(
                        imglist[indics[i]], jsonlist[indics[i]], ftr)
                    '''
                        ...
                        '''
                    # indics = np.arange(len(imglist))
                    write_listfile(imglist[indics[i]],
                                   jsonlist[indics[i]], ftrimg)  # ftrimg = open(args.csv_dir + '/trainimglist_'+str(k)+'.csv', "w")
            feval.close()
            fval.close()
            ftr.close()
            ftrimg.close()
        ...
        if args.val == 0.0:
            ...
            train_img, val_img, train_json, val_json = train_test_split(
                imglist, jsonlist, test_size=args.val, random_state=1234)
            f = open(args.csv_dir + '/traindata.csv', "w")
            ftrimg = open(args.csv_dir + '/trainimglist.csv', "w")
